...
const baseSession: Session = {
  ...
  lastUsageEventAt: "2026-01-01T00:01:00.000Z",
...
describe("usage accounting", () => {
  it("uses cumulative snapshots without adding historical totals twice", () => {
    const events: UsageEvent[] = [100, 250].map((inputTokens, index) => ({
      ...
      reportedTotalTokens: inputTokens + (index === 0 ? 10 : 30),
      hasNegativeValues: false,
      estimatedCost: null,
    }));

    expect(accountUsageEvents("codex", events, "unknown-model")).toMatchObject({
      inputTokens: 250,
      outputTokens: 30,
